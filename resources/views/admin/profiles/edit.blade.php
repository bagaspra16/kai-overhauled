...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
    <div>
        ...
        <p class="text-gray-600">Perbarui informasi profil perusahaan</p>
    </div>
...
<form action="{{ route('admin.profiles.update', $profile->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
    @csrf
    @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div class="p-6">
                    <div id="companyPreview" class="bg-gray-50 rounded-lg p-6 text-center">
                        ...
                        <h4 id="livePreviewName" class="font-semibold text-gray-900 mb-1">{{ $profile->nama_perusahaan }}</h4>
                        <p id="livePreviewSlogan" class="text-sm text-kai-blue font-medium mb-2">{{ $profile->slogan }}</p>
                        <div class="text-xs text-gray-600 space-y-1">
                            <p id="livePreviewEmail">{{ $profile->email }}</p>
                            <p id="livePreviewPhone">{{ $profile->telepon }}</p>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <span class="font-mono text-kai-blue">#{{ $profile->id }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <span class="text-sm">{{ $profile->created_at->format('M d, Y') }}</span>
                    </div>
                    
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <span class="text-gray-600">Terakhir Updated</span>
                        <span class="text-sm">{{ $profile->updated_at->format('M d, Y') }}</span>
                    </div>
                </div>
            </div>
...
<script>
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Updating Company Profile...';
